...
name: secrets
description: >-
  声明式注册模板所需的 API 密钥和服务凭证，使其出现在
  agent 侧边栏设置 UI 和入门检查清单中。用于任何第三方
  API 密钥（OpenAI、Stripe、Twilio 等）以及在统一设置
  UI 中展示 OAuth 连接。
metadata:
  internal: true
---
...
永远不要硬编码凭证值。源码、文档、测试、固件、提示、种子数据和生成的扩展/应用内容可以提及凭证**名称**（如 `OPENAI_API_KEY`），但不得包含真实的 API 密钥、令牌、Webhook URL、签名密钥、OAuth 刷新令牌或私有的 Builder/客户数据。

凭证值在运行时通过部署配置、加密的 `app_secrets` 保管库、`saveCredential` / `resolveCredential`、OAuth 或 `${keys.NAME}` 替换提供。示例必须使用明显的占位符，如 `<OPENAI_API_KEY>` 或 `${keys.SLACK_WEBHOOK}`，而非看起来真实的复制值。

## 何时使用

用于模板需要的任何外部凭证：API 密钥、服务令牌、Webhook 密钥。它提供：

- 每个凭证的侧边栏 UI 条目（掩码输入、轮换、测试、删除）。
- `required: true` 密钥的自动入门检查清单项。
- 稳定的服务端读取 API（`readAppSecret`），按需解密值。
- 验证器钩子，用于在保存前和从测试按钮检查密钥健康状况。

## 何时不使用

- 需要运行完整授权码交换的 OAuth 流程——直接使用 `@agent-native/core/oauth-tokens` 保存/刷新令牌。注册表仍可通过注册 `kind: "oauth"` 的密钥在侧边栏展示 OAuth 连接——这只是将状态查找委托给 oauth-tokens 并渲染一个连接按钮，不会写入 `app_secrets` 行。
- 纯进程级环境变量，永远不会面向用户（如 `NODE_ENV`、部署标志）。这些属于入门 `form` 方法或 `core-routes-plugin` 中的 `envKeys` 列表。
...
| `description`      | `string?`                               | 标签下的副标题。                                                |
| `docsUrl`          | `string?`                               | 卡片上渲染的"获取密钥"链接。                                     |
| `scope`            | `"user" \| "workspace"`                 | 按用户或跨活动组织共享。                                |
| `kind`             | `"api-key" \| "oauth"`                  | 驱动 UI 和存储行为。                                          |
| `required`         | `boolean?`                              | 为 true 时，自动注入入门步骤。                          |
| `validator`        | `(v) => Promise<boolean \| {ok,error}>` | 保存时和测试按钮运行。永远不要记录 `v`。                    |
| `oauthProvider`    | `string?`（仅 oauth 类型）             | `oauth-tokens` 中支持此条目的 provider id。                     |
...
- **永远不要记录值。** 读取层在服务端强制执行此规则；你的代码也必须如此。
- **仅对部署级密钥使用环境变量。** 如果凭证是用户范围、组织范围或工作区范围的，读取范围化的保管库/凭证存储。不要添加 `process.env` 回退，使每个用户继承一个部署的密钥。
- **范围与注册匹配。** `scope: "user"` → 传递用户邮箱。`scope: "workspace"` → 从 `getOrgContext(event).orgId` 传递活动的 `orgId`。

## HTTP 路由

Core 路由插件自动在 `/_agent-native/secrets/` 下挂载这些路由：

- `GET /_agent-native/secrets` — 列出已注册的密钥及其状态（`set` / `unset` / `invalid`）、元数据，以及——对于已设置的 api-key——最后 4 个字符。永远不会返回值。
- `POST /_agent-native/secrets/:key` — 请求体 `{ value, scope?, scopeId? }`。运行已注册的验证器；失败时返回 400 及错误信息。
- `DELETE /_agent-native/secrets/:key` — 删除存储的值。
- `POST /_agent-native/secrets/:key/test` — 针对当前存储的值重新运行验证器。
...
- `GET /_agent-native/secrets/adhoc` — 列出所有临时密钥（名称、最后 4 个字符、URL 允许列表）。永远不会返回值。
- `POST /_agent-native/secrets/adhoc` — 请求体 `{ name, value, urlAllowlist? }`。创建或更新临时密钥。
- `DELETE /_agent-native/secrets/adhoc/:name` — 删除临时密钥。

### URL 允许列表

每个临时密钥可以有一个 URL 允许列表——一组源 URL 数组，限制密钥值可以发送到哪里。检查是源级别的（scheme + host + port）。如果未配置允许列表，密钥可用于任何 URL。
...
`web-request` 工具支持 URL、请求头和请求体中的 `${keys.NAME}` 占位符。替换在 agent 发出工具调用后在服务端进行——原始密钥值永远不会进入 agent 的上下文。
...
密钥解析从用户范围回退到工作区范围，因此用户可以覆盖共享密钥而不破坏引用工作区默认值的自动化。

## Dispatch 保管库访问

Dispatch 工作区有一个工作区应用凭证的保管库访问策略：

- `all-apps` 是默认值。每个已保存的 Dispatch 保管库密钥对每个工作区应用可用；`sync-vault-to-app` 将所有保管库密钥推送到目标应用。
- `manual` 需要显式的按应用授权。使用 `create-vault-grant` / `grant-vault-secrets-to-app`，然后 `sync-vault-to-app`。

在决定是否创建授权之前使用 `get-vault-access-settings`，仅当用户要求更改策略时使用 `set-vault-access-settings`。
...
- `onboarding` — 必需密钥显示在其中的设置检查清单。
- `actions` — 调用第三方 API 时读取密钥的地方。
- `authentication` — 会话范围；`scope: "user"` 使用会话邮箱。
- `security` — 输入验证和永远不记录密钥。
- `automations` — 临时密钥驱动自动化 Web 请求中的 `${keys.NAME}`。