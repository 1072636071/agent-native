...
扩展的完整 HTML 示例。有关模型和使用时机概述，请参阅 `../SKILL_zh.md`。有关完整的辅助函数/全局变量 API 表，请参阅 `api_zh.md`。

## API 状态仪表板

检查多个端点的健康状况并显示绿色/红色状态：
...
## 待办列表（使用 extensionData）

使用内置 `extensionData` 存储的完整 CRUD 应用——无需 SQL、无需 schema 文件、无需 action。数据自动按扩展和按用户范围限定：
...
使用 localStorage 的持久笔记——无需 API 密钥：
