...
name: automations
description: >-
  事件触发和计划触发的自动化，支持自然语言条件。在创建自动化、接入事件或理解触发器如何触发时使用。
metadata:
  internal: true
---
...
自动化是响应事件或按 cron 计划触发的 Agent 执行任务。每个自动化是 `jobs/` 下的 Markdown 资源，带有描述何时以及如何触发的 YAML frontmatter，以及包含 Agent 遵循的自然语言指令的正文。自动化通过事件触发器、自然语言条件评估和通过 `web-request` 工具的出站 HTTP 扩展了 recurring-jobs 系统。

## 两种触发器类型

| 类型        | 触发时机                                       | 关键字段              |
| ----------- | ---------------------------------------------- | --------------------- |
| `schedule`  | Cron 表达式匹配（与 recurring jobs 相同）      | `schedule`（cron）    |
| `event`     | 事件总线上发出匹配的事件                       | `event`（事件名称）   |

事件触发器可以选择包含 `condition` — 在分派前由 Haiku 针对事件负载评估的自然语言字符串。如果条件不匹配，自动化被跳过。

## 工作原理

1. 用户请求 Agent 创建自动化（或使用设置 UI）。
2. Agent 调用 `manage-automations` 并带 `action=list-events` 来发现可用事件。
3. Agent 调用 `manage-automations` 并带 `action=define` 来写入 `jobs/<name>.md` 资源。
4. 触发器分派器订阅总线上的事件。
5. 当事件触发时，分派器加载所有匹配的触发器，通过 Haiku 评估条件，并分派匹配的触发器。
6. 在 `agentic` 模式下，分派器运行完整的 `runAgentLoop`，以自动化正文作为提示，事件负载作为上下文。
...
| 字段          | 类型                           | 用途                                                 |
| ------------- | ------------------------------ | ---------------------------------------------------- |
| `schedule`    | `string`                       | Cron 表达式（schedule 触发器必需）                   |
| `enabled`     | `boolean`                      | 自动化是否激活                                       |
| `triggerType` | `"schedule" \| "event"`        | 自动化如何触发                                       |
| `event`       | `string?`                      | 要订阅的事件名称（event 触发器）                     |
...
| `lastError`   | `string?`                      | 最后一次失败运行的错误消息                           |

## Agent 工具

所有自动化操作通过带有 `action` 参数的单个 `manage-automations` 工具访问：

| Action        | 用途                                                              |
| ------------- | ------------------------------------------------------------------ |
| `list-events` | 发现所有已注册事件及其描述和负载 schema                           |
| `list`        | 列出所有自动化及其状态，按 domain 或 enabled 过滤                 |
| `define`      | 创建新自动化（名称、触发器类型、事件、条件、正文）                |
| `update`      | 更新现有自动化（enabled、condition、body）                        |
...
当自动化有 `condition` 时，分派器调用配置的快速/分类模型来分类事件负载是否满足条件。这是一个是/否分类，不是生成任务。确切的模型 ID 在 `condition-evaluator.ts` 中。

- 空或缺失条件 = 无条件（始终触发）。
- 结果被记忆化（条件的 SHA-256 + 负载），5 分钟 TTL 和 500 条目 LRU 缓存。
- 负载在发送给 Haiku 前截断到 4000 字符。
- API 失败时，条件评估为 `false`（安全默认 — 跳过自动化）。

## `web-request` 工具和密钥

自动化使用 `web-request` 工具进行出站 HTTP。它支持 URL、头和正文中的 `${keys.NAME}` 占位符。这些在 Agent 发出工具调用后在服务端解析 — 原始密钥值永远不会进入 Agent 的上下文。

- 密钥是用户通过设置 UI 或 `/_agent-native/secrets/adhoc` API 创建的临时密钥。
- 每个密钥可以有一个 URL 允许列表，限制密钥可以发送到哪些来源。
- `resolveKeyReferences()` 解析占位符，从用户作用域回退到工作区作用域。
- `validateUrlAllowlist()` 根据每个密钥的允许列表检查解析的 URL（来源级别匹配）。
- 自动化定义、示例、事件负载和提示不得
  硬编码真实 API 密钥、Webhook URL、令牌、私有 Builder/内部数据或
  客户数据。使用 `${keys.NAME}` 和合成的 `example.com` 身份。

## UI

自动化显示在设置面板的"自动化"部分。用户可以从那里查看、启用/禁用和删除自动化。创建通常通过 Agent 聊天进行。

## 示例

用户："当有人用 @example.com 邮箱预订会议时，在 Slack 通知我。"
...
| `packages/core/src/triggers/dispatcher.ts`     | 事件订阅和 Agent 分派                             |
