...
name: server-plugins
description: >-
  框架服务端插件和 `/_agent-native/` 路由命名空间。在添加自定义服务端插件、决定是创建 `/api/` 路由还是 action，或调试自动挂载的框架路由时使用。
metadata:
  internal: true
---
...
所有框架级路由位于 `/_agent-native/` 下，以避免与模板特定的 `/api/*` 路由冲突。
...
- 永远不要将框架路由放在 `/api/` 下。
- 永远不要将模板路由放在 `/_agent-native/` 下 — 该命名空间是保留的。
- 永远不要创建仅包装、代理或重导出 action 的 `/api/*` 路由。使用现有的 `/_agent-native/actions/:name` 端点或 React action 钩子。

### 自动挂载的框架路由

| 路由                                                          | 用途                                   |
| ------------------------------------------------------------- | -------------------------------------- |
| `GET /_agent-native/poll`                                     | 用于 DB 变更检测的轮询端点             |
| `GET /_agent-native/events`                                   | 用于实时同步的 SSE 端点                |
| `GET /_agent-native/ping`                                     | 健康检查                               |
| `GET/PUT/DELETE /_agent-native/application-state/:key`        | 应用状态 CRUD                          |
| `GET/PUT/DELETE /_agent-native/application-state/compose/:id` | 撰写草稿 CRUD                          |
| `POST /_agent-native/agent-chat`                              | Agent 聊天 SSE 端点                    |
| `GET /_agent-native/agent-chat/mentions`                      | @标签的提及搜索                        |
| `GET /_agent-native/env-status`                               | 环境键配置状态                         |
| `POST /_agent-native/env-vars`                                | 保存环境变量                           |
| `/_agent-native/auth/*`                                       | 认证（登录、会话、登出）               |
| `/_agent-native/google/*`                                     | Google OAuth（回调、认证 URL 等）      |
...
| `/_agent-native/collab/*`                                     | 实时协作（参见 `real-time-collab`）    |
| `/_agent-native/a2a`                                          | A2A JSON-RPC 端点（参见 `a2a-protocol`）|

## Action 优先方法

对于标准 CRUD 和数据操作，在 `actions/` 中使用 `defineAction` — 框架将它们自动挂载为 `/_agent-native/actions/:name` 的 HTTP 端点。仅当 action 无法完成时才创建自定义 `/api/*` 路由：
...
在添加路由之前，检查现有的 action 文件。如果 action 已经编码了业务规则，则复用它；如果操作应该对 Agent 和 UI 都可用，则添加新 action。其实现主要调用 action 的路由通常是错误的抽象。

Nitro Vite 插件通过 `server/routes/` 中基于文件的路由处理 `/api/` 和 `/_agent-native/` 前缀。

## 相关技能

- `actions` — 优先使用 action 而不是自定义 `/api/` 路由
