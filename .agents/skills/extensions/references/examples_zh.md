...
扩展的工作 HTML 示例。对于模型和使用时机概述，参见 `../SKILL_zh.md`。对于完整的辅助函数/全局 API 表，参见 `api_zh.md`。
...
## 待办事项列表（使用 extensionData）

使用内置 `extensionData` 存储的完整 CRUD 应用 — 无需 SQL、无需 schema 文件、无需 action。数据自动按扩展和按用户范围化：
