...
description: >-
  ...
  在实现或回答关于 @agent-native/core API、生成的应用、工作区或
  高级功能的问题之前使用。
metadata:
  internal: true
---
...
无头 `pnpm agent` 循环和内置应用 agent 也暴露一个只读的 `docs-search` 工具，具有相同的 `query`、`slug` 和 `list` 选项。
...
| 对 agent 可见的 UI 状态 | `context-awareness` |
...
| Skill 和指令 | `skills-guide`、`writing-agent-instructions` |

## 不要

- 当包文档存在时，不要依赖记忆来获取框架 API。
