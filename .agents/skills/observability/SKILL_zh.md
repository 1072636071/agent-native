...
name: observability
description: >-
  Agent 可观测性、评估、反馈和实验。在添加可观测性仪表板、配置
  跟踪捕获、设置评估、创建 A/B 实验或收集用户对 agent 响应的
  反馈时使用。
metadata:
  internal: true
---
...
可观测性系统以零配置自动检测每个 agent 运行。跟踪、自动评估和反馈收集开箱即用。所有数据存储在应用自己的 SQL 数据库中——不需要外部服务。模板可以选择导出到 Langfuse、Datadog 或任何 OTel 兼容平台。
...
内容（prompt、工具参数、工具结果）**默认被编辑**。通过 `observability-config` 设置键选择加入：
...
**显式**——`ThumbsFeedback` 组件在聊天 UI 中的每个 agent 消息上渲染内联的竖起/竖下拇指。竖下拇指打开类别弹出框（不准确、无帮助、错误工具、太慢）。已通过 `React.lazy` 接入 `AssistantChat.tsx`。

**隐式**——`computeSatisfactionScore(threadId)` 从对话信号计算挫折指数（0-100）：
- 重述检测（权重 30）：连续相似的用户消息
- 放弃（权重 20）：会话在 agent 响应后不久结束
- 情绪（权重 15）：负面语言模式
- 长度趋势（权重 15）：消息长度递减
- 重试模式（权重 20）："再试一次"、"不，那是错的"

分数解读：0-20 健康、20-40 摩擦、40-60 不满意、60+ 崩溃。

满意度评分在每次带 threadId 的反馈 POST 后自动触发。
...
- `step_efficiency`——无工具运行为 1.0；对使用工具的运行惩罚过多的 LLM 迭代
- `latency_score`——以 10 秒/工具基线归一化
- `cost_efficiency`——以 50 centicents/工具基线归一化
- `error_recovery`——如果运行从工具错误恢复或没有错误则为 1.0

**LLM-as-judge（采样）：** 在 `evalSampleRate` 比例的运行上运行。调用配置的引擎，使用 judge prompt 按自定义标准评分。

**数据集评估：** `runDatasetEval(datasetId)` 通过 agent 运行黄金数据集并对每个案例评分。
...
上面三层在事后对*真实生产运行*评分。对于主动的确定性门控，使用 `@agent-native/core/eval` 中的一等 `*.eval.ts` 原语（源代码：`packages/core/src/eval/*`）。它对固定输入运行实际 agent 循环，低于阈值时以非零退出，因此它门控 CI/部署。
...
- 内置评分器：`exactMatch` / `contains` / `usesTool`（纯 JS）和 `llmJudge`（提供商无关的 judge）。
- 自定义评分器：`createScorer` 配合 4 步 `preprocess → analyze → generateScore → generateReason` 管道（仅 `generateScore` 是必需的）。
- 作为门控运行：`agent-native eval [pattern] [--json] [--threshold N]`——发现 `**/*.eval.ts` 和 `evals/*.ts`，运行 agent，如果任何评估低于其阈值则以非零退出。没有评估文件的应用退出 `0`。补充（不替代）`evals.ts` 中的事后评分。见评估文档。
...
Agent 循环通过 `resolveActiveExperimentConfig()` 读取活跃实验并自动应用变体的 `model` 覆盖。分配使用一致性哈希——同一用户始终获得相同的变体。

使用 `POST /_agent-native/observability/experiments/:id/results` 计算结果。

### 5. 仪表板

`ObservabilityDashboard` 是一个有 5 个标签页的 React 组件：
- **概览**——指标卡片（运行次数、成本、延迟、工具成功率、竖拇指率、评估分数）
- **对话**——跟踪列表，可钻取到 span 详情
- **评估**——评估统计和标准细分条
- **实验**——实验列表带状态徽章，可钻取到结果
- **反馈**——反馈流、竖拇指比率、类别徽章
...
- `agent_trace_summaries`——聚合的运行摘要
- `agent_feedback`——显式用户反馈
- `agent_satisfaction_scores`——计算的挫折指数
- `agent_evals`——评估结果
...
所有表是方言无关的（SQLite + Postgres）且严格增量的。
...
| `packages/core/src/observability/feedback.ts` | 反馈 + 挫折指数 |
...
框架发出与 Langfuse、Datadog、Grafana、New Relic 和任何 OTel 兼容后端兼容的 `gen_ai.*` 语义约定 span。

## 实时 OpenTelemetry Span（可选）

与上面将内部跟踪发送到 OTLP 端点的 `exporters` 配置分开，agent 循环还可以为每次运行、模型调用和工具调用发出**实时 OpenTelemetry span**，这样已经运行 OTel 收集器的主机可以与其他分布式跟踪一起看到 agent 活动。

此层是可选的且**默认无操作**：

- `@opentelemetry/api` 是**可选依赖**。如果未安装，span 辅助函数降级为静默无操作——它们永远不会抛入 agent 循环。
- 即使安装了 api 包，它也提供默认的无操作 tracer。Span 仅在**主机注册 `TracerProvider`**（通过 `@opentelemetry/sdk-node` 或类似）后才变为真实。框架故意不依赖重型 SDK/导出器包，也从不自己注册提供者——检测是嵌入应用的选择加入。

循环发出 `agent.run`（带 `agent.run_id`、`agent.thread_id`、`agent.user_id`、`agent.model`）、`tool.call`（`tool.name` + 状态）和 `llm.call` span，每个以 OK/ERROR 状态完成。这对内部的 `agent_trace_spans` / `agent_trace_summaries` 表是纯增量的。源代码：`packages/core/src/observability/tracing.ts` + `traces.ts`。见可观测性文档了解完整表格。