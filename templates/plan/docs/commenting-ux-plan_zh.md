# 计划：Agent-Native Plan 评论 UX 大修

> 独立设计计划（Opus 4.8）。目标：当人类在计划上留下评论**和**编辑并交接给编码代理时，代理获得**绝对清晰**的 (a) 确切评论了什么 — 哪些文本、图片/线框图/canvas 的哪部分，视觉评论带有聚焦标记的截图，(b) 编辑的 diff，(c) 每条评论是给**人类**还是**代理**解决的 — 加上 Figma 级别的 `@mention` 标记。
>
> 此计划旨在合并到已在进行中的实现。它被框架为**已发布的内容 → 真正的漏洞 → 修复**，带有已验证的 `file:line` 引用，以便实现代理可以精确行动。
