...
name: visual-recap
description: >-
  将 PR、分支、提交或 git diff 转化为带有图表、文件映射、API/schema 摘要、
  带注释 diff 和聚焦审查笔记的交互式可视化回顾。
metadata:
  visibility: exported
---
...
`/visual-recap` 创建一个从 diff **构建**的可视化计划，而不是朝向一个。它是正向规划的反向：不是描述你即将进行的更改，而是描述刚刚进行的更改，在更高的层次上
