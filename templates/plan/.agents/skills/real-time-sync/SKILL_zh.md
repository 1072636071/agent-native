...
name: real-time-sync
description: >-
  如何通过 SSE 加轮询后备保持 UI 与代理更改同步。
  在为新数据模型连接查询失效、调试 UI 不更新或理解抖动防止时使用。
metadata:
  internal: true
---
...
UI 通过 `useDbSync()` 与代理/脚本更改保持同步。进程内写入首先通过 `/_agent-native/events` 流式传输；`/_agent-native/poll` 仍然是跨进程/无服务器的后备。当代理写入数据库时，UI 检测到更改并自动更新 — 无需手动刷新。
