...
name: visual-plan
description: >-
  将普通文本计划转化为带有图表、文件映射、带注释代码、开放问题和
  UI/原型审查的富交互式可视化计划（在有用时）。
metadata:
  visibility: exported
---
...
Agent-Native Plans 是面向编码代理的结构化可视化规划模式。构建你通常会以 Markdown 编写的计划，但作为混合了可编辑块的可扫描文档：行内图表、代码片段、开放问题，以及可选的顶部视觉审查区域（线框图画布、实时原型，或两者在标签页中）。架构和后端计划保持仅文档；UI 和产品计划从顶部画布/原型开始（视觉表面选择部分拥有该规则）。
