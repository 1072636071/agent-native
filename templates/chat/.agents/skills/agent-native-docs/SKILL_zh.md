...
description: >-
  如何查找 node_modules 中捆绑的版本匹配的 Agent Native 框架文档。
  在实现或回答关于 @agent-native/core API、生成应用、工作区或高级功能的问题之前使用。
metadata:
  internal: true
---
...
在实现或解释非平凡的 Agent Native 行为之前，阅读与 `@agent-native/core` 一起安装的版本匹配文档。
...
无头 `pnpm agent` 循环和内置应用智能体也暴露具有相同 `query`、`slug` 和 `list` 选项的只读 `docs-search` 工具。
...
然后阅读 `node_modules/@agent-native/core/docs/AGENTS.md` 或
`node_modules/@agent-native/core/docs/content/` 下的匹配文件。
