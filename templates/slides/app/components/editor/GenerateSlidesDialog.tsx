      ...
      <DialogContent className="bg-card border-border max-w-lg">
        <DialogHeader>
          <DialogTitle className="text-foreground flex items-center gap-2">
            {t("slides.generate.title")}
          </DialogTitle>
          <DialogDescription className="text-muted-foreground">
            {t("slides.generate.description")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 mt-2">
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground block mb-1.5">
              {t("slides.generate.topic")}
            </label>
            <textarea
              value={topic}
              onChange={(e) => setTopic(e.target.value)}
              placeholder={t("slides.generate.topicPlaceholder")}
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              <label className="text-xs font-medium text-muted-foreground block mb-1.5">
                {t("slides.generate.style")}
              </label>
              <input
                value={style}
                onChange={(e) => setStyle(e.target.value)}
                placeholder={t("slides.generate.stylePlaceholder")}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-sm text-foreground/90">
                {t("slides.generate.generateImages")}
              </span>
              <p className="text-xs text-muted-foreground">
                {t("slides.generate.imagesDescription")}
              </p>
            </div>
          </div>
          ...
          {includeImages && (
            <div>
              <label className="text-xs font-medium text-muted-foreground block mb-1.5">
                ...
                {t("slides.generate.referenceImages")}
              </label>
